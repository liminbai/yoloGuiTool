...
DEFAULT_TAGS = ["raw_import", "incremental"]


def build_parser():
    parser = argparse.ArgumentParser(
        description="将新图片与 X-AnyLabeling 标注（JSON）增量追加到已有 FiftyOne 数据集（不会重复导入）。",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "命令行示例:\n"
            f"  python 01_append_data.py --dataset-name {DEFAULT_DATASET_NAME} \\\n"
            ...
            "  - 若未指定 --tags，默认使用 ['raw_import', 'incremental']；\n"
            "  - 增量导入默认不开启 --overwrite，除非你想重建整个数据集。\n"
        ),
    )
    parser.add_argument(
        "--dataset-name",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help=f"新增源图片所在目录，未指定时进入交互输入（默认: {DEFAULT_IMAGE_DIR}）",
    )
    parser.add_argument(
        "--labels-dir",
        type=str,
        default=None,
        help=f"新增 X-AnyLabeling JSON 标注所在目录，未指定时进入交互输入（默认: {DEFAULT_LABELS_DIR}）",
    )
    parser.add_argument(
        "-t",
        "--tags",
        nargs="*",
        default=None,
        help=f"附加到新增样本的标签，可传多个，如 --tags raw_import incremental（默认: {DEFAULT_TAGS}）",
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="数据集已存在时先删除再全量重新导入（重建数据集，一般增量场景不建议开启）",
    )
    return parser
