...
# PySide6 Imports
# -----------------------------------------------------------------------------
from PySide6.QtGui import (QAction, QIcon, QImage, QImageReader, QColor,
                           QKeySequence, QPixmap, QCursor, QShortcut)
from PySide6.QtCore import (Qt, QSize, QPoint, QPointF, QByteArray, QTimer,
                            QFileInfo, QProcess, Slot, Signal, QUrl)
from PySide6.QtWidgets import (QMainWindow, QVBoxLayout, QCheckBox, QHBoxLayout,
                               QWidget, QToolButton, QListWidget, QDockWidget,
                               QScrollArea, QListWidgetItem, QMessageBox, QMenu,
                               QFileDialog, QApplication, QLabel, QWidgetAction,
                               QToolBar, QComboBox, QLayout)
# -----------------------------------------------------------------------------

# 注意: libs 目录下的模块也需要迁移到 PySide6
from libs.combobox import ComboBox
from libs.default_label_combobox import DefaultLabelComboBox
from libs.resources import *  # 警告: 必须使用 pyside6-rcc 重新编译 resources.qrc
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None):
        ...
        # PySide6: QAction is in QtGui, but parent can be QObject/QWidget
        ...
        # PySide6: returns standard Python list automatically
        ...
        # PySide6: QApplication.screens() instead of desktop().screenCount()
        found_screen = False
        for screen in QApplication.screens():
            if screen.availableGeometry().contains(saved_position):
                ...
                break
        if not found_screen:
            # Fallback if position is off-screen
            position = QPoint(0, 0)
        ...
        def xbool(x):
            # PySide6 handles boolean conversion automatically, but keeping safe check
            return bool(x)
    ...
    def file_item_double_clicked(self, item=None):
        if item is None:
            return
        # Get the full path from user data
        imgPath = ustr(item.data(Qt.ItemDataRole.UserRole))
        if imgPath and imgPath in self.m_img_list:
    ...
    def zoom_request(self, delta):
        ...
        cursor = QCursor()
        # PySide6: cursor.pos() is still valid for global pos
        pos = cursor.pos()
    ...
    def import_dir_images(self, dir_path, keep_index=False):
        """
        Import images from a directory.
        
        Args:
            dir_path: Path to the directory containing images
            keep_index: If True, try to maintain the current image index
                        (used when deleting images). If False, open the first image
                        (used when opening a new directory).
        """
        if not self.may_continue() or not dir_path:
            return

        # Save current index if we want to preserve it
        saved_idx = self.cur_img_idx if keep_index and hasattr(self, 'cur_img_idx') else 0
        ...
        # Add files to file_list_widget BEFORE opening the first image
        for imgPath in self.m_img_list:
            # Display only the filename, not the full path
            filename = os.path.basename(imgPath)
            item = QListWidgetItem(filename)
            item.setData(Qt.ItemDataRole.UserRole, imgPath)  # Store full path in user data
            self.file_list_widget.addItem(item)
        
        # Open the appropriate image based on keep_index flag
        if keep_index and self.img_count > 0:
            # Try to maintain current index, stay on the same image if possible
            self.cur_img_idx = min(saved_idx, self.img_count - 1)
            filename = self.m_img_list[self.cur_img_idx]
            self.load_file(filename)
        elif self.img_count > 0:
            # Open the first image, used when opening a new directory
    ...
    def delete_image(self):
        ...
        for imgPath in self.m_img_list:
            filename = os.path.basename(imgPath)
            item = QListWidgetItem(filename)
            item.setData(Qt.ItemDataRole.UserRole, imgPath)
            self.file_list_widget.addItem(item)
        
        # Load the image at the same index (which now points to the next image)
        if self.img_count > 0:
            new_idx = min(idx, self.img_count - 1)
            self.cur_img_idx = new_idx
            filename = self.m_img_list[new_idx]
            self.load_file(filename)
        else:
            # No images left
            self.close_file()
